...
class AsociarRetoState(SuscriptorMixin, AuthState):
    """Modo Asociar: agregar retos existentes a un evento destino (carrito)."""
    busqueda: str = ""
...
class GestionarRetoState(SuscriptorMixin, AuthState):
    """Modo Gestionar: editar / quitar los retos que ya tiene un evento."""
    busqueda_gest: str = ""
    ...
    def set_id_evento_gest(self, v: str):
        ...
        return GestionarRetoState.cargar_gestion
    ...
    def cargar_todo(self):
        ...
        self.id_evento_gest = ""
        self.retos_gest = []
        self.id_categoria_gest_filtro = self.id_dificultad_gest_filtro = self.id_modo_gest_filtro = ""
        catalogo = asociar.cargar_catalogos()
        self.categorias = catalogo["categorias"]
        self.modos = catalogo["modos"]
        self.dificultades = catalogo["dificultades"]
    ...
    @rx.event(background=True)
    async def escuchar_gestionar(self):
        await suscriptor.escuchar(
            self,
            clave_getter=lambda s: "gestionar",
            canales_getter=lambda s: [canales.CH_RETOS, canales.CH_EVENTOS],
            recargar=lambda s: s._refrescar_vivo(),
        )

    @rx.event
    def parar_gestionar(self):
        self.streaming = False
...
class EditarAsociarState(AuthState):
    ...
    async def guardar_edit_contiene(self):
        ...
        gestion = await self.get_state(GestionarRetoState)
